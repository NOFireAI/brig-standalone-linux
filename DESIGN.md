...
- **Split layout, reversing the single-root decision.** Per #1272: `/opt/brig`
  holds the immutable binaries, config and boot assets; `/var/lib/brig` holds
  the containerd store, snapshots and pool; `/run/brig` holds the socket. The
  earlier "one base directory" call was right for a bare urunc stack but #1272
  wants the immutable/mutable split, so brig takes it.
- **One tarball, built by the release, unpacked by the installer.** The unit of
  ...
  workflow. It is a complete `/opt/brig` tree: every binary, every config file,
  the systemd units, the wrappers and the boot assets, laid out and ready.
...
- **The installer's own job** is the host wiring the tarball cannot carry: create
  the state and socket dirs, set up the device-mapper pool (or fall back to
  overlayfs and retarget the two config files), create the `brig` group, drop the
...
- **Non-root via a group.** A `brig` group owns the private containerd socket: the
  ...
  sources `/opt/brig/etc/brig-env.sh` (pointing nerdctl at the private containerd
...
shim, nerdctl, static VMMs, everything under `/opt/brig` and `/var/lib/brig`,
...
`pins.env` is the manifest #1272 asks for. It is also what the SBOM job reads,
and what `urunc-ctl version` prints.
...
- **P4 -- release.** A workflow that builds the per-arch tarballs, publishes
  `install.sh`, records `pins.env`, signs and attaches an SBOM per the
  supply-chain policy in #1277.
