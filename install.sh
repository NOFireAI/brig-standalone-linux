...
# laid out as an /opt/brig tree. install.sh unpacks it and wires it into the host:
...
PREFIX="/opt/brig"          # immutable binaries, config, boot assets
DATA_DIR="/var/lib/brig"    # mutable state: containerd store, snapshots, pool
...
Fetches the brig release tarball and installs it into /opt/brig, with state in
/var/lib/brig and the socket in /run/brig. It builds nothing.
  ...
  INSTALL_BRIG_FORCE          true to take over an /opt/brig we did not create
  INSTALL_BRIG_SNAPSHOTTER    devmapper | overlayfs (default: devmapper)
...
setup_env() {
    ...
    SNAPSHOTTER="${INSTALL_BRIG_SNAPSHOTTER:-devmapper}"
    SNAPSHOTTER_PINNED=false
    [ -n "${INSTALL_BRIG_SNAPSHOTTER:-}" ] && SNAPSHOTTER_PINNED=true
...
verify_system() {
    ...
    if [ "$SNAPSHOTTER" = "devmapper" ]; then
        for tool in dmsetup losetup blockdev; do
            command -v "$tool" >/dev/null 2>&1 || fatal "'$tool' is required for the devmapper snapshotter"
        done
        modprobe dm_thin_pool 2>/dev/null || true
        # A kernel without CONFIG_DM_THIN_PROVISIONING has no thin-pool target.
        if ! dmsetup targets 2>/dev/null | grep -q '^thin-pool'; then
            if [ "$SNAPSHOTTER_PINNED" = "true" ]; then
                fatal "this kernel has no device-mapper thin-pool target.
    $(uname -r) was built without CONFIG_DM_THIN_PROVISIONING. Use
    INSTALL_BRIG_SNAPSHOTTER=overlayfs."
            fi
            warn "no device-mapper thin-pool target on this kernel, using overlayfs."
            warn "the guest rootfs then goes over virtiofs (cloud-hypervisor, qemu)."
...
unpack_tarball() {
    ...
    for f in pins.env components.json; do
        [ -f "$root/$f" ] && cp "$root/$f" "$PREFIX/$f"
    done
    chmod 0755 "$PREFIX"

    mkdir -p "$DATA_DIR/containerd" "$DATA_DIR/nerdctl" "$DATA_DIR/log" "$RUN_DIR"

    # The tarball's containerd.toml and nerdctl.toml are written for devmapper.
    # When the host fell back to overlayfs, retarget those two files.
    if [ "$SNAPSHOTTER" = "overlayfs" ]; then
        sed -i "s/snapshotter = 'devmapper'/snapshotter = 'overlayfs'/g" "$ETC_DIR/containerd.toml" 2>/dev/null || true
        sed -i 's/^snapshotter = "devmapper"/snapshotter = "overlayfs"/' "$ETC_DIR/nerdctl.toml" 2>/dev/null || true
        say "configuration retargeted to the overlayfs snapshotter"
    fi
}
...
setup_storage() {
    ...
    POOL_LOOP_META=""
    if [ "$SNAPSHOTTER" != "devmapper" ]; then
        say "snapshotter is $SNAPSHOTTER, no pool needed"
        return 0
    fi
...
POOL_NAME=$([ "$SNAPSHOTTER" = "devmapper" ] && echo "$POOL_NAME" || echo "")
...
install_systemd() {
    ...
    if [ "$SNAPSHOTTER" = "devmapper" ] && [ -f "$unit_dir/$POOL_SERVICE_NAME.service" ]; then
...
start_stack() {
    ...
    if [ "$SNAPSHOTTER" = "devmapper" ]; then
        ...
        [ "$state" = "ok" ] || fatal "the devmapper snapshotter is not ok. Check the pool $POOL_NAME."
    fi
    say "containerd up, snapshotter $SNAPSHOTTER ok"
}
